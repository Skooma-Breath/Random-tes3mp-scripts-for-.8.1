...
--TODO help info msg in chat if a player lives in the house you enter about how to enter a clean instance of the vanilla cell...(create a command for such instancing)
--TODO display if a house is for sale and for how much in the showAllHousesList menu...
...
local function reload_housingData(pid, cmd)
	Load()
end
...
local function OnPlayerAuthentifiedHandler(eventStatus, pid)
	if not housingData.coOwners then
		housingData.coOwners = {}
	end
	if not housingData.coOwners[getName(pid)] then
			housingData.coOwners[getName(pid)] = {}
	end
end
...
local function addCoOwner(houseName, pname)
	...
	local hdata = housingData.houses[houseName]
	-- housingData.coOwners = {}
	-- housingData.coOwners[pname] = {}

	if pname ~= oname then
		housingData.owners[oname].houses[houseName].coowners[pname] = true

		if not housingData.coOwners[pname] then
			housingData.coOwners[pname] = {}
	  end

		if tableHelper.containsValue(housingData.coOwners[pname], houseName) then
			-- tes3mp.MessageBox(pid, -1, "Player is already a co-owner.")
			return
		end

		table.insert(housingData.coOwners[pname], houseName)
...
local function removeCoOwner(houseName, pname)
	...
	--TODO remove the coOwners house from the list of houses in housingData.coOwners[pname]
		tableHelper.removeValue(housingData.coOwners[pname], houseName)
	-- housingData.coOwners[pname] = {}
...
showPlayerSettingsRemoveList = function(pid)
	...
	if not housingData.owners[getName(pid)] then
		tes3mp.MessageBox(pid, -1, color.LimeGreen .. "You can't do that.\n")
		return
	end
...
showPlayerSettingsOwnedList = function(pid)
	...
	if housingData.coOwners then
		for co_OwnerName, houseName in pairs(housingData.coOwners) do
			if getName(pid) == co_OwnerName then
				for i, v in pairs(houseName) do
...
local function onPlayerOwnedHouseSelect(pid, index)
		if playerOwnedHouseList[getName(pid)][index] == nil then
			playerSelectedHouse[getName(pid)] = housingData.coOwners[getName(pid)][index]
		else
...
showPlayerSettingsMain = function(pid)
	...
	if playerSelectedHouse[getName(pid)] and getHouseOwnerName(playerSelectedHouse[getName(pid)]) ~= getName(pid) then
		if not tableHelper.containsValue(housingData.coOwners[getName(pid)], playerSelectedHouse[getName(pid)]) then
...
--TODO different strings with the same length appear different lengths in the listbox....
...
local function onHouseCreatePrompt(pid, data)
	--TODO: Checks? --yes checks.
	if data == " " or data == nil then tes3mp.MessageBox(pid, -1, color.Warning .. "Canceled house creation.")return end
	createNewHouse(data)
...
Methods.OnGUIAction = function(pid, idGui, data)
	...
	elseif idGui == config.HouseInfoGUI then -- House Info
		if tonumber(data) == 0 then --Buy
			...
			--back to house list
			onPlayerMainList(pid) --TODO not returnin to house list????
			return true
		end
...
customCommandHooks.registerCommand("loadjson", reload_housingData)
customCommandHooks.setRankRequirement("loadjson", 2)
