...
--TODO make a custom record weapon for selecting refid's of objects with OnObjectHit and puts them directly into a specific defined category table.
...
--Special Containers
furnitureData["special containers"] = {
...
-- tes3mp.LogAppend(enumerations.log.INFO, "------------------------- " .. furnitureData.chests[1].name)
-- -- tableHelper.print(furnitureData)

--TODO make command that loads the json files into memory
local function saveCat()
	jsonInterface.save("custom/category_list.json", furnitureCategories)
end
local function loadCat()
	furnitureCategories = jsonInterface.load("custom/category_list.json")
end

local function saveFurn()
	jsonInterface.save("custom/furniture_list.json", furnitureData)
end
local function loadFurn()
	furnitureData = jsonInterface.load("custom/furniture_list.json")
end
local function createFurnJson()
  if jsonInterface.load("custom/furniture_list.json") ~= nil then
		loadFurn()
	else
		saveFurn()
	end
end
local function createCatJson()
  if jsonInterface.load("custom/category_list.json") ~= nil then
		loadCat()
	else
		saveCat()
	end
...
createFurnJson()
createCatJson()
-- jsonInterface.save("custom/fuck.json", furniturecrafting)
...
local function getFurnitureData(refId)
	tes3mp.LogAppend(enumerations.log.INFO, "------------------------- " .. "the god dam mother fucking refId: " .. tostring(refId))
...
local function getSortedPlayerFurnitureInventory(pid)
	...
	for refId, amount in pairs(inv) do
		tes3mp.LogAppend(enumerations.log.INFO, "------------------------- " .. "STINKY POOP TURD CRAPS" .. refId .. "----" .. amount)
	...
	if sorted ~= nil then
		...
		tes3mp.LogAppend(enumerations.log.INFO, "------------------------- " .. "smells like shit in this bitch")
		return
	end
end
...
local function placeFurniture(refId, loc, cell)
  ...
  objectData.count = 0
  objectData.charge = -1
  objectData.enchantmentCharge = -1
  objectData.soul = ""
	local uniqueIndex = logicHandler.CreateObjectAtLocation(cell, location, objectData, "place")
...
local function getAvailableFurnitureStock(pid)
	...
	for i = 1, #furnitureData do
		tes3mp.LogAppend(enumerations.log.INFO, "------------------------- " .. "1 inside getAvailableFurnitureStock: ")
		table.insert(options, furnitureData[i])
	end
	for k, v in ipairs(furnitureData) do
		tes3mp.LogAppend(enumerations.log.INFO, "------------------------- " .. "2 inside getAvailableFurnitureStock: ")
	end

	tes3mp.LogAppend(enumerations.log.INFO, "------------------------- " .. "WHAT IN THE ABSALUTE FUCK IS GOING ON????")
	return options
end
...
-- Methods.mergeFurniture = function(furniture, category)
--
-- 		tes3mp.LogAppend(enumerations.log.INFO, "------------------------- " .. "mergeFurniture from kanaFurniture")
-- 		-- tableHelper.merge(furnitureData, furniture)
-- 		table.insert(furnitureCategories, category)
-- 		furnitureData[category] = furniture
--
--
-- 		tableHelper.print(furnitureCategories)
--
-- 		tableHelper.print(furnitureData)
--
-- end
--
-- Methods.categoryList = function(pid)
-- 		local list = ""
-- 		for i, v in pairs(furnitureCategories) do
-- 				list = list .. tostring(v) .. "\n"
-- 		end
-- 		tes3mp.ListBox(pid, config.categoryList, "Select a category.\nSelect nothing to cancel.", list)
-- end
--
-- Methods.categoryThingDoer = function(data)
--
-- 		for i = 0, #furnitureCategories do
-- 				if tonumber(data) == i then
-- 						category = tostring(furnitureCategories[i+1])
...
showViewOptionsGUI = function(pid, loc)
	...
	--TODO sort by closest object to player
	...
	if options and #options > 0 then
		for i = 1, #options do
			...
			if object then
				...
				list = list .. tostring(furnData.name) .. " (at " .. math.floor(object.location.posX + 0.5) .. ", "  ..  math.floor(object.location.posY + 0.5) .. ", " .. math.floor(object.location.posZ + 0.5) .. ")"
...
showInventoryGUI = function(pid)
	...
	local list = "* CLOSE *\n"
	-- if options == nil then
	-- 	tes3mp.LogAppend(enumerations.log.INFO, "------------------------- " .. "PISS BALLS SHIT FUCK")
	-- 	tes3mp.ListBox(pid, config.InventoryGUI, "You haven't purchased any furniture yet.", list)
	-- 	return
	-- end
	for i = 1, #options do
		-- tes3mp.LogAppend(enumerations.log.INFO, "------------------------- " .. "974: " .. tostring(options[i].name))
...
showBuyGUI = function(pid)
	...
	table.sort(furnitureCategories, function(a,b) return a < b end)

	for i = 1, #furnitureCategories do
		list = list .. furnitureCategories[i]
		if not(i == #furnitureCategories) then
			list = list .. "\n"
		end
	end

	playerBuyOptions[getName(pid)] = furnitureCategories
...
local function buyItem(pid, loc)
	...
	tes3mp.LogAppend(enumerations.log.INFO, "------------------------- " .. "poop")
	-- playerBuyOptions[getName(pid)] = list
...
local function onBuyChoice(pid, loc)
	...
	local choice = playerBuyOptions[getName(pid)][loc]
	if choice == nil then
		tes3mp.LogAppend(enumerations.log.INFO, "------------------------- " .. "choice was: " .. tostring(choice))
		return false
	end
	-- TODO add tes3mp.InputDialog for amount of item to buy.
	if pgold < tonumber(choice.price) then
...
showMainGUI = function(pid)
	...
	tes3mp.CustomMessageBox(pid, config.MainGUI, message, color.Green .. "Buy;" .. color.Green .. "Inventory;" .. color.Green .. "View;" .. color.Red .. "Close")
end
...
local function toggleSelectionMode(pid, cmd)
		Players[pid].data.customVariables.addFurnMode = not Players[pid].data.customVariables.addFurnMode
		if Players[pid].data.customVariables.addFurnMode then
				mode = true
				tes3mp.MessageBox(pid, -1, "Addfurn mode was enabled.")
		else
				mode = false
				tes3mp.MessageBox(pid, -1, "Addfurn mode was disabled.")
		end
end
...
local temp = {} --TODO test moving these inside the append_object function.
local category
-- local temp = {name = cmd[2], refId = cmd[3], price = cmd[4]}
-- make this better....
Methods.append_object = function(pid, cmd) --TODO add a command for removing/rm furniture items
	if cmd[2] == "category" then
		if tableHelper.containsValue(furnitureCategories, cmd[3]) then
			category = cmd[3]
		else
			tes3mp.SendMessage(pid, config.InvalidCategoryMsg)
			return
		end
		for i = 4, #cmd do
      if cmd[i] then
        category = category .. " " .. cmd[i]
      else
        break
      end
    end
  ...
  if cmd[2] == "name" then
    ...
    for i = 4,10,1 do
      if cmd[i] then
        temp.name = temp.name .. " " .. cmd[i]
      else
        break
      end
    end
  end
  if cmd[2] == "refid" or cmd[2] == "refId" then
    temp.refId = cmd[3]
    for i = 4,10,1 do
      if cmd[i] then
        temp.refId = temp.refId .. " " .. cmd[i]
      else
        break
      end
    end
  ...
  if cmd[2] == "addfurn" and Players[pid]:IsServerStaff() then --TODO sort the before saving
		tes3mp.LogAppend(enumerations.log.INFO, "------------------------- " .. "category: ".. tostring(category))
		if category == nil then
			tes3mp.SendMessage(pid, "Enter a category\n")
			return
		elseif temp.refId == nil then
			tes3mp.SendMessage(pid, "Enter a refId\n")
			return
		elseif temp.name == nil then
			 tes3mp.SendMessage(pid, "Enter a name\n")
			 return
		elseif temp.price == nil then
			tes3mp.SendMessage(pid, "Enter a price\n")
			return
		end
    table.insert(furnitureData[category], 1, temp)
    saveFurn()
    tes3mp.SendMessage(pid, "You have added " .. tostring(temp.refId) .. " to the list of furniture\n", false)

  elseif not Players[pid]:IsServerStaff() then
    tes3mp.SendMessage(pid, "Your not cool enough", false)
  end
end

Methods.AddCategory = function(pid, cmd)
	local cat = cmd[2]
	for i = 3, #cmd do
		if cmd[i] then
			cat = cat .. " " .. cmd[i]
	...
	table.insert(furnitureCategories, cat)
	saveCat()
	furnitureData[cat] = {}
	saveFurn()
	tes3mp.LogAppend(enumerations.log.INFO, "------------------------- " .. "AddCategory: ".. tostring(furnitureCategories[1]))
end

Methods.OnServerPostInitHandler = function()

		local weaponRecordStore = RecordStores["weapon"]
    if weaponRecordStore.data.permanentRecords["furn_selection_tool"] == nil then
        weaponRecordStore.data.permanentRecords["furn_selection_tool"] = {

            name = "Furniture Selection Tool",
            subtype = 0,
            value = 666,
            weight = 0,
            icon  = "w\\tx_w_StendarsHammer.dds",
	          model = "w\\W_StendarsHammer.NIF",
            health = -1,
            speed = 2,
            reach = 999,
            damageThrust = {
        ...
        tes3mp.LogAppend(enumerations.log.INFO, "------------------------- " .. "furniture selection tool was created.")
    else
        tes3mp.LogAppend(enumerations.log.INFO, "------------------------- " .. "poop")
    end
...
tableHelper.merge(Methods.furnitureCategories, furnitureCategories)

Methods.mergeFurn = function()

		tableHelper.merge(furnitureData, Methods.furnitureData, true)
		tableHelper.merge(furnitureCategories, Methods.furnitureCategories, true)
		tableHelper.print(furnitureData)
		tableHelper.print(furnitureCategories)
...
local function printCategories(pid, cmd)
		tableHelper.print(Methods.furnitureCategories)
end

customCommandHooks.registerCommand("pc", printCategories)
customCommandHooks.registerCommand("ac", Methods.AddCategory)
customCommandHooks.registerCommand("addfurn", Methods.append_object)
customCommandHooks.registerCommand("af", Methods.append_object)
customCommandHooks.registerCommand("furniture", Methods.OnCommand)
customCommandHooks.registerCommand("furn", Methods.OnCommand)
customCommandHooks.registerCommand("afm", toggleSelectionMode)

customEventHooks.registerValidator("OnObjectHit", Methods.OnObjectHitValidator)
customEventHooks.registerValidator("OnObjectActivate", Methods.OnObjectActivateValidator)

customEventHooks.registerHandler("OnPlayerAuthentified", Methods.OnPlayerAuthentifiedHandler)
customEventHooks.registerHandler("OnServerPostInit", Methods.OnServerPostInitHandler)
