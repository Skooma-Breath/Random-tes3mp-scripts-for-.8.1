...
local function placeFurniture(refId, loc, cell)
  ...
  objectData.count = 0
  objectData.charge = 0
  objectData.enchantmentCharge = 0
  objectData.soul = 0
...
-- BUY (MAIN)
showBuyGUI = function(pid) 	--TODO make category buttons that lead to more lists.
...
-- make this better....
Methods.append_object = function(pid, cmd)
  if cmd[2] == "name" then
    temp.name = cmd[3]
    for i = 4,10,1 do
      if cmd[i] then
  ...
  if cmd[2] == "addfurn" and Players[pid]:IsServerStaff() then
    table.insert(furnitureData, 1, temp)
    Save()
